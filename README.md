...
2. Install dependencies (Don't use npm)

```
yarn install```

3. Enter your Solana wallet private key in the `.env` file (Please protect your private key, and if running on a server, ensure it is secure.).
...
4. Enter your RPC link (After completing, remember to double-check that the `.example` suffix has been removed.)
...
5. Run `yarn start` to start the Config Wizard
6. Follow the steps in the Config Wizard
